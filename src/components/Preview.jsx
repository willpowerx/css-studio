...
(function(){
  ...
  function hideOverlays(){['css-studio-hover','css-studio-select'].forEach(function(id){var el=document.getElementById(id);if(el)el.style.display='none'})}
  ...
  document.addEventListener('mouseover',function(e){
    if(!window.__cssStudioEnabled){hideOverlays();return}
    if(isStudio(e.target))return
    const ov=getOverlay('css-studio-hover','border:1px dashed #6b7280;')
    pos(ov,e.target)
  })
  document.addEventListener('mouseout',function(){
    const ov=document.getElementById('css-studio-hover')
    if(ov)ov.style.display='none'
  })
  document.addEventListener('click',function(e){
    if(!window.__cssStudioEnabled)return
    if(isStudio(e.target))return
    e.preventDefault();e.stopPropagation()
    const ov=getOverlay('css-studio-select','border:2px solid #3b82f6;')
    pos(ov,e.target)
    window.parent.postMessage({type:'css-studio-select',selector:sel(e.target)},'*')
  },true)
  ...
  document.addEventListener('contextmenu',function(e){
    ...
    if(isStudio(e.target))return
    e.preventDefault()
    window.__cssStudioSelected=e.target
    window.parent.postMessage({type:'css-studio-contextmenu',selector:sel(e.target),x:e.clientX,y:e.clientY},'*')
  })
})()
<\/script>`
